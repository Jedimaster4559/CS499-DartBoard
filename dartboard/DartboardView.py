...
class DartboardView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
         ...
         self.points.append([(event.pos().x() - (self.WindowSizeX / 2.0)) / self.radius, (event.pos().y() - (self.WindowSizeY / 2.0)) / self.radius])

         #this line draws the point on the screen where the mouse was clicked
         self.scene.addEllipse(self.WindowPosX + event.pos().x(), self.WindowPosY + event.pos().y(), 3,3, QPen(), QBrush(Qt.white))
    ...
    def DrawRegions(self):
        ...
        for i in range(self.regions):
            dpoints = []
            endpoint = QPoint()
            for j in range(i * (self.regionVertices // self.regions), i * (self.regionVertices // self.regions) + (self.regionVertices // self.regions) + 1):
                dpoints.append(outer_bullseye_points[j])
            for j in range(i * (self.regionVertices // self.regions) + (self.regionVertices // self.regions), i * (self.regionVertices // self.regions) - 1, -1):
